> **Historical README, preserved 24 September 2026.** This is the longer account
> that preceded the short introduction. The body is preserved, with relative
> links adjusted for this location. It includes statements since qualified by
> review: publication was still pending; the cost was reconstructed, not a
> verified bill; the five perspectives do not map one-to-one to schema objects;
> and catalogue override sections were still empty. Use the
> [current README](../../README.md) for current status.
